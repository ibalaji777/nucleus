<template>
 <div style="background: white">
  <v-dialog width="700" v-model="$store.state.dialog.empLoginWidgetDialog">
   <v-card>
    <v-toolbar>
     <v-toolbar-title>Employee/credential</v-toolbar-title>
     <v-spacer></v-spacer>
     <v-btn
      icon
      style="color: black"
      @click="
       $store.commit('setDialog', { key: 'empLoginWidgetDialog', value: false })
      "
     >
 ...
 methods: {
  async employeeLogin() {
   ...
   if (result.data.success) {
    ...
    $vm.$store.dispatch("dialog", {
     key: "empLoginWidgetDialog",
     value: false,
    });
   }
  },
